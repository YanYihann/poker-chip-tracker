...
export function setupSocketServer(httpServer) {
    ...
    io.on("connection", (socket) => {
        ...
        socket.on("room:subscribe", async (rawPayload) => {
            try {
        ...
        socket.on("room:unsubscribe", async (rawPayload) => {
            try {
                const payload = subscribePayloadSchema.parse(rawPayload);
        ...
        socket.on("room:set-ready", async (rawPayload) => {
            try {
        ...
        socket.on("room:start", async (rawPayload) => {
            try {
                const payload = subscribePayloadSchema.parse(rawPayload);
        ...
        socket.on("room:action", async (rawPayload) => {
            try {
        ...
        socket.on("disconnect", async () => {
            await Promise.all(Array.from(subscribedRoomCodes).map(async (roomCode) => {
                await setPlayerConnectionByRoomCode({
                    roomCode,
                    userId: auth.userId,
                    isConnected: false
